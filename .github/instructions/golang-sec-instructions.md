...
Use the `golangci-lint` tool with the `gosec` linter to analyze the changed code:

```sh
golangci-lint run --disable-all --enable gosec <changed-files>
```
...
The full list of gosec rules can be found in the [here](https://raw.githubusercontent.com/securego/gosec/refs/heads/master/RULES.md).

## Report Deprecated Dependencies

If changes are made to `go.mod` or `go.sum`, check for any dependencies that are marked as deprecated in their module repository (e.g., GitHub). Note that deprecation is a strong signal of unmaintained code and potential security risks, especially if the deprecation notice mentions security issues.

## Run govulncheck

You will have access to the `govulncheck` tool.

Run it against the changed packages to detect known CVEs in the module dependency graph:
...
Report any vulnerabilities found: CVE ID, affected symbol, affected package, and whether the vulnerable code path is reachable from the changed code.

If `govulncheck` reports a vulnerability in a transitive dependency that the PR did not introduce, note it but mark it as pre-existing.
