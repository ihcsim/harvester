...
├── Makefile                           # All build targets (Docker-based via Dapper)
...
| Build system | Docker multi-stage builds via `Makefile` (Dapper-compatible) |
...
**Code generation** — run after any change to CRD types in `pkg/apis/`:

```bash
go generate ./...
# Pipeline: pkg/codegen/cleanup → rm pkg/generated → pkg/codegen/main.go → scripts/generate-manifest → scripts/generate-openapi
```
...
Global settings are exported `Setting` variables in `pkg/settings/`. Read values with `.Get()` — never hardcode setting values in controllers.
